    ...
    <dataTable :headings="tableHeaders" :setting="true">
        ...
        <template v-slot:data>
            <tr v-for="(item, index) in dataList.data" :key="item.id">
                <td>{{index+1}}</td>
                <td>{{ item.type == 1 ? 'Generator' : item.type == 2 ? 'Solar' : '' }}</td>
        ...
        <fromModal @submit="submitForm({
            ...
            <div class="row mb-2">
                ...
                <div class="col-md-8">
                    <input type="number" v-model="formObject.unit_rate" v-validate="'required'" step="0.01"  class="form-control"/>
                </div>
            </div>
            <div class="row mb-2">
                <label class="col-md-4"><strong>Customer Unit(Per kwh): </strong></label>
                <div class="col-md-8">
                    <input type="number" v-model="formObject.customer_unit" v-validate="'required'" step="0.01"  class="form-control"/>
                </div>
            </div>
